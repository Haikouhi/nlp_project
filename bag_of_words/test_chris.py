...
def main() :
    ...
    # placement du hashtag recherché après %23
    data = requests.get(" https://api.twitter.com/1.1/search/tweets.json?q=%23ivg&lang=fr&result_type=mixed&count=100", headers=headers)

    # création d'un fichier data.json avec tout le contenu brut
    with open('data.json', 'w') as outfile:
        json.dump(data.json(), outfile, ensure_ascii=False, indent=2)

    # récup de la data au format dictionnaire et du contenu de la clé statues
    ...
    print("Total sans stop words : ", len(tweets_flatten_splitted))

    # calcul du nombre d'occurent  de chaque mot
    wordfreq = []
    for w in tweets_flatten_splitted:
        wordfreq.append(tweets_flatten_splitted.count(w))
    print("Total des occurences : ", len(wordfreq))

    # association clef valeur pour le mot et l'occurence de celui-ci
    result = dict((zip(tweets_flatten_splitted, wordfreq)))

    # tri du dictionnaire par valeur afin d'avoir les mots ayant la plus grand occurence en premier
    ...
    plt.axis("off")
    plt.savefig('ivg7.png', bbox_inches='tight')
